...
class AIChatService {
    ...
    try {
      ...
      final requestData = {
        ...
        if (historyMessages != null && historyMessages.isNotEmpty)
          'history_messages': historyMessages.map((msg) => msg.toJson()).toList(),
      };
  ...
  /// 开始新的对话会话
  Future<ChatModel> startNewChat(String message) async {
...
class ChatSession {
  ...
  /// 发送消息
  Future<ChatModel> sendMessage(String userMessage) async {
    try {
      // 添加用户消息到本地历史
      ...
      if (sessionId == null) {
        // 首次对话
        response = await _chatService.startNewChat(userMessage);
      } else {
        // 继续对话
